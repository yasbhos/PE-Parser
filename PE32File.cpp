...
PE32File::PE32File(char* arg_name, FILE* arg_pefile) {
...
DWORD PE32File::GetOffset(DWORD arg_va) {
...
char* PE32File::GetName(DWORD arg_va) {
...
void PE32File::ParseDOSHeader() {
	...
	fread(&pfDOSHeader, sizeof(pfDOSHeader), 1, pfPEFile);
...
void PE32File::ParseExportDirectory() {
...
void PE32File::PrintFileInfo() {
	...
	printf(" File Name\t| %s\n", pfName);
	printf(" File Type\t| Portable Executable 32\n");
...
void PE32File::PrintFileHeaderInfo() {
...
void PE32File::PrintDataDirectoriesInfo(DWORD arg_offset) {
...
void PE32File::PrintOptionalHeaderInfo() {
	...
	printf(" BaseOfData\t\t\t| 0x%08X\t| DWORD\t| 0x%08X\n", lOffset, pfNTHeaders.OptionalHeader.BaseOfData);
	lOffset += sizeof(pfNTHeaders.OptionalHeader.BaseOfData);

	printf(" ImageBase\t\t\t| 0x%08X\t| DWORD\t| 0x%08X\n", lOffset, pfNTHeaders.OptionalHeader.ImageBase);
...
void PE32File::PrintExportTableInfo() {
...
void PE32File::PrintImportTableInfo() {
	...
	for (int i = 0; i < pfImportDirectoryCount; i++)
	{
		...
		while (true)
		{
			...
			fseek(pfPEFile, lOffset, SEEK_SET);
			fread(&lTemp, sizeof(DWORD), 1, pfPEFile);
		...
		{
			DWORD lILTEntry, lIATEntry;

			fseek(pfPEFile, (lILTOffset + (j * sizeof(DWORD))), SEEK_SET);
			fread(&lILTEntry, sizeof(DWORD), 1, pfPEFile);

			fseek(pfPEFile, (lIATOffset + (j * sizeof(DWORD))), SEEK_SET);
			fread(&lIATEntry, sizeof(DWORD), 1, pfPEFile);

			DWORD lFlag = lILTEntry & 0x80000000;
			DWORD lHintRVA = 0x0;
			WORD lOrdinal = 0x0;
			...
			{
				printf("\t0x%08X\t| 0x%08X\t| N/A\t\t| Ordinal: 0x%08X\n", lILTEntry, lIATEntry, lOrdinal);
			}
		}
